...
const HomePage = () => {
  ...
  const getWeather = async() => {
    try {
      const res = await axios.get(`http://api.weatherapi.com/v1/current.json?key=${import.meta.env.VITE_WEATHER_API}&q=${city}`)
      setWeather(res?.data)
    } catch (error) {
      console.log(error);
    }
      ...
      <div className="w-full flex flex-col-reverse py-5 lg:flex-row justify-center">
        ...
        <div className="w-full lg:w-2/5 relative min-h-11 md:min-h-14">
          <img
            ...
            alt=""
          />
          <div className="w-[90%] sm:w-md m-1 backdrop-blur-md absolute top-4 left-1/2 -translate-x-1/2 rounded-xl p-3">
            <div className="flex justify-between items-center p-2">
              <div className="flex justify-center gap-1 items-center">
                <MdOutlineAddLocationAlt />
                <span className="flex justify-center items-center gap-3 dark:text-gray-900">{weather?.location?.name} {" "} {" "} <span className="text-xs font-bold"> {time.currDate} {time.currMonth} {time.curYear}</span> </span>
              </div>
              <span><sup>o</sup>c/<sup>o</sup>F</span>
            </div>
            {/* temprature */}
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-center pt-6 dark:text-gray-900">{weather?.current?.temp_c} <sup>o</sup>C  <span className="text-xs" >{time.hours}:{time.minutes}:{time.seconds}.{time.ampm} {time.day}</span> </h1>
              <p className="text-2xl font-semibold text-center text-gray-800 dark:text-gray-900" >Cloudy</p>

            <div className="flex flex-row sm:flex-row justify-center items-center gap-6 py-5">
              <div className="flex flex-col items-center gap-1">
                <p className="text-sm font-bold tracking-tighter dark:text-gray-900">Humidity</p>
                <span className="text-sm font-bold ">{weather?.current?.humidity}</span>
              </div>
              <div className="flex flex-col items-center gap-1">
                <p className="text-sm font-bold tracking-tighter dark:text-gray-900">Wind</p>
                <span className="text-sm font-bold dark:text-gray-900 ">{weather?.current?.wind_kph} km/h</span>
              </div>
              <div className="flex flex-col items-center gap-1">
                <p className="text-sm font-bold tracking-tighter dark:text-gray-900">Feels Like</p>
                <span className="text-sm font-bold dark:text-gray-900">{weather?.current?.feelslike_c} <sup>o</sup></span>
              </div>
            </div>
